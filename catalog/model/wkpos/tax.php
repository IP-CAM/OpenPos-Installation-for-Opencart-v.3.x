<?php
class ModelWkposTax extends Model {
  /**
  * used for tax management
  */
  public function getTax($value, $tax_class_id, $tax_rates) {
    $amount = 0;

    $tax_rates_1 = $this->getRates($value, $tax_class_id, $tax_rates);

    foreach ($tax_rates_1 as $tax_rate) {
  ...
  * used for tax management
  */
  public function setTaxZone($country_id, $zone_id, $shipping = false, $payment = false) {
    $tax_rates = array();
  ...
  * used for tax management
  */
  public function getTaxes($tax_rates) {
    $tax_data = array();

    foreach ($this->cart->getProducts() as $product) {
      if ($product['tax_class_id']) {
        $tax_rates_1 = $this->getRates($product['price'], $product['tax_class_id'], $tax_rates);
